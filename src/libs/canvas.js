...
const localCanvas = class localCanvas {
    ...
    _renderStarrySky () {
        ...
        // 绘制画布
        this.ctx.fillRect(0, 0, this.setting.canvasStyle.width, this.setting.canvasStyle.height);

        this.edges.forEach ((item) => {
            // 计算出两点间的距离(勾股定理)
            let len = Math.sqrt(Math.pow((item.from.x - item.to.x), 2) + Math.pow((item.from.y - item.to.y), 2));
            // 给点之间设置一个参考距离
            let threshold = this.setting.distance || this.setting.canvasStyle.width / 8;
    ...
    _createParticulates (item) {
        ...
        if (!item.boomed) {
            ...
            for (let i = 0; i < item.count; i++) {
                ...
                particulate.vx = particulate.radius * particulate.v * Math.sin(particulate.angle);
                particulate.vy = particulate.radius * particulate.v * Math.cos(particulate.angle);
    ...
    _boom () {
        ...
        this.particulates.forEach((item1) => {
            item1.forEach((item2, index ,arr) => {
                ...
                let max = Math.max(Math.abs(item2.x - item2.x0), Math.abs(item2.y - item2.y0));
                if (max > item2.size) {
                    // 超过上述的最大距离则移除该点
                    arr.splice(index, 1);
                    return;
     ...
     */
    drawCircle (x = 0, y = 0, radius, m = 0, n = 2 * Math.PI, counterclockwise = false) {
        //开始路径
        this.ctx.beginPath();
